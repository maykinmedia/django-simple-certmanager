...
class SigningRequest(DeleteFileFieldFilesMixin, models.Model):
    ...
    csr = models.TextField(editable=False, blank=True)
        ...
        if not self.csr:
            ...
            if self.email_address:
                csr_builder = csr_builder.add_extension(
                    x509.SubjectAlternativeName([x509.RFC822Name(self.email_address)]),
                    critical=False,
                )
            ...
            with self.private_key.open(mode="rb") as key_f:
                private_key = serialization.load_pem_private_key(
                    key_f.read(), password=None, backend=default_backend()
                )
    ...
    def generate_private_key(self):
        if not self.private_key:
            ...
            private_key_file = ContentFile(
                private_key_file_bytes, name="private_key.pem"
            )
    ...
    def create_certificate(self, certificate):
        created_certificate = Certificate.objects.create(
            label="Certificate for " + self.common_name,
            type=CertificateTypes.key_pair,
            signing_request=self,
